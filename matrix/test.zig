...
test "Matrix Random fills values in range" {
...
test "Matrix Mul DimensionMismatch" {
    const a = m.Matrix(2, 3){ .data = [_]f32{1, 2, 3, 4, 5, 6} };
    const b = m.Matrix(2, 2){ .data = [_]f32{7, 8, 9, 10} };
    try std.testing.expectError(m.DimensionMismatch, a.SMul(2, 2, &b));
}
...
test "Matrix Mul" {
    ...
    const a = Mat2x2{ .data = [_]f32{ 2, 6, 5, 4 } };
    const b = Mat2x2{ .data = [_]f32{ 2, 3, 2, 8 } };
    const expected = Mat2x2{ .data = [_]f32{ 16, 54, 18, 47 } };
